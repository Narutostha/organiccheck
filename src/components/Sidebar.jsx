        ...
        <div className="space-y-3 sm:space-y-4">
          <div>
            ...
            <input
              ...
              max="1000"
              defaultValue={range}
              onMouseUp={(e) => setRange(parseInt(e.target.value, 10))}
              onTouchEnd={(e) => setRange(parseInt(e.target.value, 10))}
  ...
  return (
    <>
      <button
        ...
        className="fixed top-20 left-0 z-50 bg-lightGreen p-2 rounded-full shadow-md md:hidden"
      >
        <BiRightArrowAlt size={28} className="text-darkGreen" />
      </button>
      ...
      <div
        className={`fixed inset-0 bg-opacity-10 z-40 transition-opacity duration-300 md:hidden ${
          isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
        onClick={toggleSidebar}
      />

      {/* Mobile sidebar drawer with highest z-index */}
      <div
        className={`fixed top-16 left-0 h-fit w-64 sm:w-72 bg-white z-50 transform transition-transform duration-300 ease-in-out md:hidden ${
